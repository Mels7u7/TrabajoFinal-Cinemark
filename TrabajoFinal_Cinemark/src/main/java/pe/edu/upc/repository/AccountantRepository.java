...
public interface AccountantRepository extends JpaRepository<Accountant, Integer> {
	...
	List<Accountant> findByName(@Param("name") String name);

	@Query("select count(c.documentNumber) from Accountant c where c.documentNumber =:documentNumber")
	public int findByDocument(@Param("documentNumber") String documentNumber);
	...
	public void update(@Param("name") String name, @Param("accountingInstitution") String accountingInstitution,
